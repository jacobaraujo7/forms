...
class Estado extends Equatable {
  ...
  const Estado(this.sigla, this.nome, this.cidades);

  static Estado withName(name) {
    return Estado('', name, const []);
  }

  static List<Estado> getAllStates() {
    final map = jsonDecode(_json);
    return (map['estados'] as List).map(Estado.fromMap).toList();
  }
  ...
  factory Estado.fromMap(dynamic map) {
    return Estado(
      map['sigla'] ?? '',
      map['nome'] ?? '',
      List<String>.from(map['cidades']),
    );
  }
